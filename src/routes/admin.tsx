...
function AdminPage() {
  ...
  useEffect(() => {
    ...
    if (user.role !== "admin") { navigate({ to: "/account" }); return; }
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="pt-24">
        <section className="mx-auto max-w-6xl px-6 py-16">
          <SectionHeader eyebrow="Admin Panel" title="Manage TailorHub" description="Users, catalogue and fitting requests, all in one place." />
            ...
            {TABS.map((t) => (
              <button
                ...
                className={`rounded-full px-4 py-2 text-sm transition-colors ${
                  tab === t ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:text-gold"
                }`}
              >
                {t}
              </button>
        ...
        <div key={u.id} className="flex flex-wrap items-center justify-between gap-3 p-4">
          ...
          <div className="flex items-center gap-3">
            <select
              ...
              onChange={(e) => roleMutation.mutate({ id: u.id, role: e.target.value as api.UserRole })}
  ...
  return (
    <div>
      <p className="mb-4 text-sm text-muted-foreground">
        ...
        <a href="/seller" className="text-gold hover:underline">Seller Dashboard</a> — it has full create/edit forms and works for admins too.
      </p>
      <div className="divide-y divide-border/60 rounded-2xl border border-border/60">
        {products.map((p) => (
          <div key={p.id} className="flex items-center justify-between p-4">
            <div>
              ...
              <div className="text-xs text-muted-foreground">{p.category} · ${p.price} · seller #{p.sellerId ?? "house"}</div>
            </div>
            <button
        ...
        {services.map((s) => (
          <div key={s.id} className="flex items-center justify-between p-4">
            <div>
              ...
              <div className="text-xs text-muted-foreground">{s.duration} · ${s.price} · seller #{s.sellerId ?? "house"}</div>
            </div>
            <button
...
function TestimonialsTab() {
  ...
  const { data: testimonials = [], isLoading } = useQuery({ queryKey: ["testimonials"], queryFn: api.getTestimonials });
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="glass-panel mt-4 grid gap-4 rounded-2xl p-6 sm:grid-cols-2">
          <div>
            ...
            <input name="name" required className="mt-2 w-full rounded-lg border border-border bg-secondary px-4 py-3 text-sm outline-none focus:border-gold" />
          </div>
          <div>
            <label className="text-xs uppercase tracking-widest text-gold">Role</label>
            <input name="role" required placeholder="e.g. CEO, Whitcomb & Sons" className="mt-2 w-full rounded-lg border border-border bg-secondary px-4 py-3 text-sm outline-none focus:border-gold" />
          </div>
          <div className="sm:col-span-2">
            <label className="text-xs uppercase tracking-widest text-gold">Quote</label>
            <textarea name="quote" rows={3} required className="mt-2 w-full rounded-lg border border-border bg-secondary px-4 py-3 text-sm outline-none focus:border-gold" />
          </div>
          <div className="sm:col-span-2">
            <button type="submit" disabled={createMutation.isPending} className="rounded-full bg-primary px-6 py-2.5 text-sm font-medium text-primary-foreground disabled:opacity-60">
              Create
            </button>
          </div>
        </form>
...
function AppointmentsTab() {
  const queryClient = useQueryClient();
  const { data: appointments = [], isLoading } = useQuery({ queryKey: ["admin", "appointments"], queryFn: api.getAllAppointments });

  const statusMutation = useMutation({
    mutationFn: ({ id, status }: { id: number; status: string }) => api.updateAppointmentStatus(id, status),
    onSuccess: () => {
    ...
    <div className="divide-y divide-border/60 rounded-2xl border border-border/60">
      {appointments.map((a) => (
        <div key={a.id} className="flex flex-wrap items-center justify-between gap-3 p-4">
          <div>
            <div className="font-medium">{a.name} — {a.fittingType}</div>
